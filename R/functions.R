...
# generate a base population with individuals of a certain genotype
CreatePopulation = function(popName="Red", nIndividuals=100, generation=0){
  ...
  indList = lapply(1:nIndividuals, function(i)
    Individual(generation = generation,
    chromosome1 = chrom, chromosome2 = chrom))
  return(Population(members = indList, alleles = NULL))
}

# add an allele to all the individuals contained in that population
AddAllele = function(population, selectionAllele, alleleFreq) {
  ...
  alleleA = Allele(location = selectionAllele@location,
                   type = toupper(selectionAllele@type))
  allelea = Allele(location = selectionAllele@location,
                   type = tolower(selectionAllele@type))
  # getting which genotype each individual will be
  indPerPop = round(c(nIndivid * freq$fAA,
                    nIndivid * freq$fAa, nIndivid * freq$faa))
  ...
  for (i in 1:nIndivid) {
    if (genot[i] == "AA") {
      ...
      indList[[i]]@chromosome2@alleles =c(indList[[i]]@chromosome2@alleles,
                                           allelea)
    } else return("There is an unrecognized genotype")
  }
...
MatePopulations.par = function(popList, nChildren = 100,
                               cpc = 2, lambda = 2, mc.cores = 1) {
  ...
  generateChildren = function(i) {
    ...
    children = sapply(1:i, function(x) {
      MateIndividuals(parent1 = parents[[1]],
      parent2 = parents[[2]],
      lambda = lambda)})
    return(children)
  }
